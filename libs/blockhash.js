...
var blockhash = function(src, imageData, bits, method) {
    var xhr;
    ...
    return new Promise((resolve, reject) => {
		xhr.onload = function() {
	        ...
	        data = Buffer.from(xhr.response || xhr.mozResponseArrayBuffer);
	        contentType = xhr.getResponseHeader('content-type');
	        try {
              let funcs = [PNG.sync.read, jpeg.decode] 
              if (imageData.type == "jpg") funcs =  [jpeg.decode, PNG.sync.read];
            
              try {
                  imgData = funcs[0](data)
              } catch (ex) {
                  imgData = funcs[1](data);
              } 

	            resolve(blockhashData(imgData, bits, method));
	        } catch (err) {
	            reject("Couldn't decode image <" + src + "> " + contentType + ' ' + err);
	        }
	    };

	    xhr.onerror = function(err) {
	        reject("Couldn't decode image <" + src + ">");
	    };
